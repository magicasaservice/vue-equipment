...
import { functions } from '@vue-equipment/metadata'
...
function getFunctionLink(fn: string) {
  ...
  return info?.docs?.replace(/https?:\/\/maas\.engineering\//g, '/')
}
</script>
...
<template>
  <div class="grid grid-cols-[100px_auto] gap-2 text-sm mt-4 mb-8 items-start">
    ...
    <template v-if="info?.package !== 'core' && info?.package !== 'shared'">
      ...
      <div>
        <code>@maas/vue-equipment/{{ info?.package }}</code>
      </div>
    </template>
    ...
    <template v-if="info?.alias?.length">
      ...
      <div flex="~ gap-1 wrap">
        <code v-for="(a, idx) of info?.alias" :key="idx" class="!py-0">{{
          a
        }}</code>
      </div>
    </template>
