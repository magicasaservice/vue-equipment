...
describe('useScrollTo', () => {
  describe('getScrollPosition()', () => {
    ...
    it('returns window scroll position for window', () => {
      const { getScrollPosition } = useScrollTo()
      expect(getScrollPosition(window as unknown as Element)).toEqual({ x: 0, y: 0 })
    })
  })
  ...
  describe('getScrollDuration()', () => {
    it('returns zero duration when already at the target position', () => {
      ...
      expect(getScrollDuration({ parent: makeScrollable(), left: 0, top: 0, speed: 500 })).toEqual({ x: 0, y: 0 })
    })

    it('calculates duration proportional to distance', () => {
      const { getScrollDuration } = useScrollTo()
      const el = makeScrollable()
      const near = getScrollDuration({ parent: el, left: 0, top: 100, speed: 500 })
      const far = getScrollDuration({ parent: el, left: 0, top: 1000, speed: 500 })
    ...
    it('calculates lower duration for higher speed', () => {
      const { getScrollDuration } = useScrollTo()
      const el = makeScrollable()
      const slow = getScrollDuration({ parent: el, left: 0, top: 500, speed: 100 })
      const fast = getScrollDuration({ parent: el, left: 0, top: 500, speed: 1000 })
